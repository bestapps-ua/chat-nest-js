...
import { RESPONSE_SERVICE } from '../constants/response.constants';
...
export class ResponseTransformInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<Dto> {
        ...
        return next.handle().pipe(
            mergeMap(async (data: Dto[]|CursorListDto) => {
                if (
                ...
                let isList = false;
                if(data instanceof CursorListDto) {
                    method = `get${name}CursorList`;
                    isList = true;
